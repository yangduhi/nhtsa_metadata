...
FastAPI는 로컬 asset console을 `/`에서 제공한다. 정적 파일은 `src/nhtsa_metadata/api/static/`에 있으며, 브라우저 UI는 `/api/download-assets`, `/api/download-jobs`, `/api/download-jobs/{job_id}/run`을 호출한다. Asset 목록은 active job(`queued`/`running`) 상태를 함께 노출하므로 이미 queue에 있는 행은 GUI에서 `Queued #<job_id>`로 비활성화된다.
