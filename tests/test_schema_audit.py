...
def test_schema_audit_reports_no_fixture_canonical_duplicates(tmp_settings) -> None:  # type: ignore[no-untyped-def]
    ...
    assert duplicate_groups["vehicles"] == []
    assert duplicate_groups["test_participants"] == []
    assert duplicate_groups["barriers"] == []
    assert any(
        "[*]" in row["field_path"]
...
def test_schema_audit_cli_writes_json(tmp_settings, tmp_path: Path) -> None:  # type: ignore[no-untyped-def]
    ...
    result = CliRunner().invoke(
        app,
        ["schema", "audit", "--database-url", tmp_settings.database_url, "--output", str(output)],
    )
